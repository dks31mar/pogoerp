...
<div class="row">
	...
	<div class="page-heading col-sm-11"
		...
		<span class="glyphicon glyphicon-user"></span> Customer Status
		 <label
			 style="margin-left: 250px;margin-top: 8px;"><button 
			class="btn btn-primary" id="getcustomerpopup" style="margin-bottom: -25px;margin-top: -26px;HEIGHT: 28px;margin-left: 561px;"> Add Customer Status </button>
     </label> 
	...
	<table class="responstable" style="margin-left: 22px; ">
		...
		<tbody>
			...
			<c:if test="${!empty customerLevelsList}">
				<c:forEach items="${customerLevelsList}" var="customerlevel" varStatus="loop">

					<tr>
						<td>${loop.index+1}</td>
						<td>${customerlevel.status}</td>
		                <%-- <td>${customerlevel.level}</td>  --%>
						 <td><a href="#" onclick="editCur(${customerlevel.id})" title="Edit" align="center" >
