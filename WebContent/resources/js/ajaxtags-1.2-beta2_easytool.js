      ...
      for (var i=0; i<itemNodes.length; i++) {
        ...
        if (nameNodes.length > 0 && valueNodes.length > 0) {
          ...
          var value = valueNodes[0].firstChild.nodeValue;
            ...
            //Sarvesh 09-05-2012 purpose: lov decoration
            var rowClass = "" ;
            /*if((i % 2) == 0)
                rowClass = "alternateColor1" ;
            else*/
                rowClass = "alternateColor2" ;

          ul += '<li id="' + value + '" class="' + rowClass + '">' + name + '</li>';
        }
      }
    }
    ul += '</ul>';
...
AjaxJspTag.HtmlContent.prototype = Object.extend(new AjaxJspTag.Base(), {
  ...
  execute: function(e) {
    ...
    // parse parameters and do replacements
// Changes by Harish Chand Date: 02/May/2012
...
/*vikas 24/oct/2008 add try catch in ajax script for removing script error while adding new customer in service,order*/
try{
